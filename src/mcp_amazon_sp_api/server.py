...
def get_product_details(asin: str, marketplace: str = "") -> str:
    ...
    Args:
        asin: El ASIN del producto (ej: "B0XXXXXXX1")
        marketplace: Código marketplace (ES, DE, FR, IT, GB). Vacío = default de .env.
    """
    try:
        client = _get_client(marketplace)
...
def get_product_type_info(product_type: str = "", keywords: str = "", marketplace: str = "") -> str:
    ...
    Args:
        product_type: Tipo exacto (ej: "WATER_BOTTLE") para ver su definición. Vacío = buscar.
        keywords: Palabras clave para buscar tipos de producto (ej: "water bottle", "water bottle")
        marketplace: Código marketplace (ES, DE, FR, IT, GB). Vacío = default de .env.
    """
    try:
        client = _get_client(marketplace)
...
def update_listing_attribute(
    ...
    Args:
        ...
        product_type: Tipo de producto (ej: "WATER_BOTTLE"). Usar get_product_type_info para encontrarlo.
...
def get_search_performance(asins: str, marketplace: str = "") -> str:
    ...
    Args:
        asins: ASINs a analizar separados por coma (ej: "B0XXXXXXX1,B0XXXXXXX2"). Máx ~10 ASINs.
        marketplace: Código marketplace (ES, DE, FR, IT, GB). Vacío = default de .env.
    """
    try:
        client = _get_client(marketplace)
...
def get_inventory(sku: str = "", marketplace: str = "") -> str:
    ...
    Args:
        sku: SKU específico (ej: "WB-500-SLV"). Vacío = todos los SKUs. Separar múltiples con coma (máx 50).
        marketplace: Código marketplace (ES, DE, FR, IT, GB). Vacío = default de .env.
    """
    try:
        client = _get_client(marketplace)
...
def get_competitive_pricing(asins: str, marketplace: str = "") -> str:
    ...
    Args:
        asins: ASINs separados por coma (ej: "B0XXXXXXX1,B0XXXXXXX2"). Máximo 20.
        marketplace: Código marketplace (ES, DE, FR, IT, GB). Vacío = default de .env.
    """
    try:
        client = _get_client(marketplace)
...
def analyze_competitor_prices(keywords: str, max_results: int = 10, marketplace: str = "") -> str:
    ...
    Args:
        keywords: Términos de búsqueda (ej: "stainless steel water bottle 500ml", "Acme Ultra Clear Case")
...
def compare_with_competitors(my_asin: str, keywords: str, max_results: int = 10, marketplace: str = "") -> str:
    ...
    Args:
        my_asin: Tu ASIN para usar como referencia (ej: "B0XXXXXXX1")
        keywords: Términos de búsqueda para encontrar competidores (ej: "insulated water bottle 1L")
...
def bulk_update_prices(updates: str, marketplace: str = "", confirm: bool = False) -> str:
    ...
    Args:
        updates: JSON array con los SKUs y precios. Ejemplo:
            [{"sku": "WB-500-SLV", "price": 14.99}, {"sku": "WB-750-BLK", "price": 12.99}]
        marketplace: Código marketplace (ES, DE, FR, IT, GB). Vacío = default de .env.
        confirm: Debe ser True para ejecutar. False = solo muestra el plan.
    """
    try:
