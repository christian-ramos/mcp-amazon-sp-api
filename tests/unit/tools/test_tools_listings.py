...
class TestGetListingContent:
    def test_extracts_attributes(self, mock_client):
        mock_client.get_listing_item.return_value = {
            "sku": "SKU-001",
            "summaries": [{"asin": "B001", "status": "BUYABLE", "productType": "PHONE_CASE"}],
            "attributes": {
                "item_name": [{"value": "Funda iPhone 16 Pro"}],
                "bullet_point": [{"value": "Protección total"}, {"value": "Material premium"}],
                "product_description": [{"value": "La mejor funda"}],
                "generic_keyword": [{"value": "funda iphone proteccion"}],
            },
            "issues": [], "offers": [],
        }
        ...
        assert result["description"] == "La mejor funda"
...
class TestListMyListings:
    def test_returns_listings(self, mock_client):
        mock_client.search_listings_items.return_value = [{
            "sku": "SKU-001",
            "summaries": [{"asin": "B001", "itemName": "Funda A", "status": "BUYABLE", "productType": "PHONE_CASE"}],
...
class TestGetProductTypeInfo:
    def test_returns_definition(self, mock_client):
        ...
        result = parse(get_product_type_info(product_type="PHONE_CASE"))
    ...
    def test_searches_by_keywords(self, mock_client):
        mock_client.search_product_types.return_value = [{"name": "PHONE_CASE", "displayName": "Phone Case", "marketplaceIds": []}]
        result = parse(get_product_type_info(keywords="phone case"))
        assert result[0]["name"] == "PHONE_CASE"


class TestUpdateListingAttribute:
    def test_without_confirm_returns_plan(self, mock_client):
        result = parse(update_listing_attribute(sku="SKU-001", product_type="PHONE_CASE", attribute_name="item_name", value="Nuevo título"))
    ...
    def test_updates_single_attribute(self, mock_client):
        ...
        result = parse(update_listing_attribute(sku="SKU-001", product_type="PHONE_CASE", attribute_name="item_name", value="Nuevo título", confirm=True))
    ...
    def test_handles_json_array_for_bullets(self, mock_client):
        ...
        update_listing_attribute(sku="SKU-001", product_type="PHONE_CASE", attribute_name="bullet_point", value='["Bullet 1", "Bullet 2"]', confirm=True)
    ...
    def test_error_handling(self, mock_client):
        ...
        assert "error" in parse(update_listing_attribute(sku="SKU-001", product_type="PHONE_CASE", attribute_name="item_name", value="test", confirm=True))


class TestUpdateListingBatch:
    def test_without_confirm_returns_plan(self, mock_client):
        updates = json.dumps({"item_name": "Nuevo título", "bullet_point": ["B1", "B2"]})
        result = parse(update_listing_batch(sku="SKU-001", product_type="PHONE_CASE", updates=updates))
        assert result["confirmed"] is False
    ...
    def test_updates_multiple_attributes(self, mock_client):
        ...
        result = parse(update_listing_batch(sku="SKU-001", product_type="PHONE_CASE", updates=updates, confirm=True))
        assert result["status"] == "ACCEPTED"
    ...
    def test_invalid_json_returns_error(self, mock_client):
        result = parse(update_listing_batch(sku="SKU-001", product_type="PHONE_CASE", updates="not json"))
        assert "JSON inválido" in result["error"]

    def test_error_handling(self, mock_client):
        mock_client.patch_listing_item.side_effect = RuntimeError("API error")
        assert "error" in parse(update_listing_batch(sku="SKU-001", product_type="PHONE_CASE", updates='{"item_name": "test"}', confirm=True))
